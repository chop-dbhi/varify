...
cd /opt/apps/harvest-app/continuous_deployment/data_service/data/ && mkdir varify-demo-data-0.1 && tar -xvf varify-demo-data-0.1.tar.gz -C varify-demo-data-0.1
cd /opt/apps/harvest-app/continuous_deployment/data_service/data/ && gzip -d varify-demo-data-0.1/*data*/*.gz

cd /opt/apps/harvest-app && DJANGO_SETTINGS_MODULE=varify.conf.settings ./bin/manage.py rqworker variants &
cd /opt/apps/harvest-app && DJANGO_SETTINGS_MODULE=varify.conf.settings ./bin/manage.py rqworker default &
