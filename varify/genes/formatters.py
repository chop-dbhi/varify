...
class HGNCGene(HTMLFormatter):
    ...
    def to_html(self, values, **context):
        if values['hgnc_id']:
            ...
            return '<a target=_blank href="{href}">{label}</a>'.format(href=href, label=values['symbol'])
    ...
    def to_excel(self, values, **context):
        if values['hgnc_id']:
            href = self.href.format(values['hgnc_id'])
            return '=HYPERLINK("{href}", "{label}")'.format(href=href, label=values['symbol'])
        return values['symbol']
    to_excel.process_multiple = True


class NCBIGene(HGNCGene):
    href = 'http://www.ncbi.nlm.nih.gov/sites/entrez?term={}&cmd=search&db=gene'
...
class RefSeqTranscript(HTMLFormatter):
    ...
    def to_html(self, value, **context):
        if value:
            ...
            return '<a target=_blank href="{href}">{label}</a>'.format(href=href, label=value)

    def to_excel(self, value, **context):
        if value:
            href = self.href.format(value)
            return '=HYPERLINK("{href}", "{label}")'.format(href=href, label=value)
