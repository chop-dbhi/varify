...
define([
    ...
    var Summary = Marionette.ItemView.extend({
        ...
        serializeData: function() {
            ...
            if (data.raw_read_depth) {
                data.raw_read_depth_class = utils.depthClass(data.raw_read_depth);
                data.raw_read_depth = '' + data.raw_read_depth + 'x';
            }
            else {
                data.raw_read_depth = 'n/a';
                data.raw_read_depth_class = 'muted';
            }

            if (data.quality) {
                data.quality_class = utils.qualityClass(data.quality);
            }
            else {
                data.quality = 'n/a';
            }

            data.genotype_value = data.genotype_value || 'n/a';

            var bases = [];
            for (var key in data.base_counts || {}) {
                bases.push('' + key + '=' + data.base_counts[key]);
            }

            if (bases.length) {
                data.base_counts = bases.sort().join(', ');
            }
            else {
                data.base_counts = 'n/a';
            }
            ...
            for (var i = 0; i < data.variant.uniqueGenes.length; i++) {
                ...
                if (gene.hgnc_id) {
                    genes.push('<a title="' + name + '" target=_blank href="http://www.genenames.org/data/hgnc_data.php?hgnc_id=' + gene.hgnc_id + '">' + gene.symbol + '</a>');
                }
                else {
