...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        # Loop through the assessments. For each assessment, we check to see
        # which sample the assessment is in. If there is no analysis already
        # created for that sample, we create it and then assign the assessment
        # to that analysis or we simply assign the assessment to the analysis
        # for its sample. We set the name and status of the analysis to give
        # it a reasonable identifier and account for the required analysis
        # status column.
        for assessment in orm.Assessment.objects.all():
            # If an assessment somehow already has a analysis, then we go
            # ahead and ignore this one.
            if assessment.analysis:
                continue

            sample = assessment.sample_result.sample

            try:
                analysis = orm['analyses.Analysis'].objects.get(sample=sample)
            except orm['analyses.Analysis'].DoesNotExist:
                analysis = orm['analyses.Analysis'].objects.create(
    ...
    def backwards(self, orm):
        # This is not reversible because we have no way of telling which
        # Analyses were created as part of this forwards migration portion and
        # which were created via user interaction.
    ...
    models = {
        ...
        'assessments.assessment': {
            ...
            'analysis': ('django.db.models.fields.related.ForeignKey', [], {'to': "orm['analyses.Analysis']", 'null': 'True', 'blank': 'True'}),
            'assessment_category': ('django.db.models.fields.related.ForeignKey', [], {'to': "orm['assessments.AssessmentCategory']"}),
            'created': ('django.db.models.fields.DateTimeField', [], {'auto_now_add': 'True', 'blank': 'True'}),
            'evidence_details': ('django.db.models.fields.TextField', [], {'null': 'True', 'blank': 'True'}),
            ...
            'status': ('django.db.models.fields.CharField', [], {'default': "'Draft'", 'max_length': '10', 'null': 'True', 'blank': 'True'}),
